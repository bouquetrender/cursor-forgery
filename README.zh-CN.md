...
5. 使用树视图或 CodeLens 接受、拒绝或请求修改。需要手动重置基线时，可随时运行 **Agent Review: Start Session**。

## 侧边栏

- **Current Turn**：尚未处理的变更。Accept 或 Reject 后会消失。
- **All Agent Changes**：当前会话的只读历史，默认收起。新增和删除文件作为只读的整文件变更显示在这里，不会进入 Current Turn。同一位置的新变更会覆盖旧记录；新会话开始时清空。
